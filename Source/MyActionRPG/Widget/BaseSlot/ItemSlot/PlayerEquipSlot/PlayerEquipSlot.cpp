...
void UPlayerEquipSlot::NativeConstruct()
{
	...
	// 우클릭, 좌 더블 클릭 시 장비 아이템을 장착 해제
	OnMouseRightButtonClicked.AddLambda([this](ESlotType slotType)
		{
			DetachToEquipItemSlot();
		});

	OnMouseLeftButtonDBClicked.AddLambda([this](ESlotType slotType)
		{
			DetachToEquipItemSlot();
		});
}
...
void UPlayerEquipSlot::DetachToEquipItemSlot(int32 inventorySlotIndex)
{
	...
	// inventorySlotIndex 가 INDEX_NONE 이거나, inventorySlotIndex 번째 인벤토리 슬롯에 아이템을 추가할 수 없는 상태라면
	// 추가 가능한 첫 번째 슬롯에 아이템을 추가함
	if (inventorySlotIndex == INDEX_NONE)
	{
		// 착용중이던 장비 아이템을 인벤토리에 추가함
	...
	{
		// inventorySlotIndex 번째 슬롯에 이전에 장착했던 장비 아이템을 추가함
		GetManager(UPlayerManager)->GetPlayerInfo()->InventoryItemInfos[inventorySlotIndex] = prevEquippedItemInfo;

		// 인벤토리 창에 추가된 아이템 슬롯들을 갱신함
