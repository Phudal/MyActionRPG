...
UPlayerInventory::UPlayerInventory()
{
	...
	if (BP_PLAYER_INVENTORY_WND.Succeeded())
...
UPlayerInventoryWnd* UPlayerInventory::CreateInventoryWnd(UWidgetControllerWidget* widgetController,
	EInputModeType changeInputMode, bool bShowCursor)
{
	...
	PlayerInventoryWnd = Cast<UPlayerInventoryWnd>(
		widgetController->CreateWnd(BP_PlayerInventory, true, changeInputMode, bShowCursor));
...
UPlayerEquipItemWnd* UPlayerInventory::CreateEquipItemWnd(UWidgetControllerWidget* widgetController,
	EInputModeType changeInputMode, bool bShowCursor)
{
	...
	if (IsValid(PlayerInventoryWnd))
	{
		...
		// 장비 창의 위치를 설정함
		PlayerEquipItemWnd->GetCanvasPanelSlot()->SetPosition(equipItemWndHalfSize);
	}
...
bool UPlayerInventory::AddItem(FItemSlotInfo& newItemSlotInfo)
{
		...
		[](FItemSlotInfo& newItemSlotInfo, TArray<FItemSlotInfo>& inventoryItemInfos, int32 slotIndex)
	{
		...
		{
			// 추가할 수 있는 여유 공간을 매꾸며, 아이템을 최대한 메움
			for (int32 i = 0; (i < addableItemCount) && newItemSlotInfo.ItemCount > 0; i++)
			{
				// 아이템을 추가함
	...
	for (int32 i = 0; i < playerInfo->InventorySlotCount; i++)
	{
		...
		if (newItemSlotInfo.ItemCount == 0)
		{
			...
			FPlayerBehaviorData bhData(EPlayerBehavior::BH_GetItem, newItemSlotInfo.ItemCode, prevItemCount);
	...
	if (prevItemCount != newItemSlotInfo.ItemCount)
	{
		...
		FPlayerBehaviorData bhData(EPlayerBehavior::BH_GetItem, newItemSlotInfo.ItemCode, addedItemCount);
		playerBehaviorBroadcast.AddBehavior(bhData);
	}
...
void UPlayerInventory::RemoveItem(int32 itemSlotIndex, int32 removeCount)
{
	...
	// 지정한 슬롯의 아이템을 removeCount 만큼 뺌
	/// removeCount 에 ITEM_ALL 이 전달되었다면 완전히 비움
	inventoryItemInfo[itemSlotIndex].ItemCount -= (removeCount == ITEM_ALL) ?
		inventoryItemInfo[itemSlotIndex].ItemCount : removeCount;

	// 아이템 개수가 0 이하라면 슬롯을 비움
...
void UPlayerInventory::EquipItem(FName equipItemCode, FItemSlotInfo* out_PrevItemSlotInfo)
{
	...
	// 이전 아이템 정보를 전달받을 변수가 전달됨
	if (out_PrevItemSlotInfo)
	{
		// 이전에 장착된 아이템 정보를 얻음
		bool bReturnedAvailableData;
		auto prevEquipItemData = 
			GetManager(UPlayerManager)->GetPlayerInfo()->GetEquippedItemSlotInfo(
			equipItemInfo->PartsType, &bReturnedAvailableData);

		// 이전에 장착된 장비가 존재하지 않는다면
		// 기본 장비라면 이전에 장비가 존재하지 않음
		...
		// 이전에 장착도니 장비가 존재한다면, 장비 정보를 반환함
		else
			*out_PrevItemSlotInfo = prevEquipItemData.Get<0>();
	}
...
void UPlayerInventory::DismountItem(EPartsType partsType, FItemSlotInfo* out_ItemSlotInfo)
{
	...
	if (out_ItemSlotInfo)
	{
		...
		// 이전에 장착된 장비가 존재한다면
			...
			*out_ItemSlotInfo = prevEquipItemData.Get<0>();
	}
...
{
	// 장착 가능한 모든 파츠 타입을 얻음
	TArray<EPartsType> equipItemPartsType;
	GetManager(UPlayerManager)->GetPlayerInfo()->PartsInfos.GetKeys(equipItemPartsType);
	/// - TMap.GetKeys(TArray<KeyType>* outKeys) : TMap 요소의 모든 키를 배열로 반환함
	/// - outKeys : 출력용 매개 변수이며, TMap 요소들의 키를 저장할 배열을 전달함

	// 비쥬얼 갱신
	for (auto partsType : equipItemPartsType)
		UpdateCharacterVisual(partsType);
}
...
void UPlayerInventory::UpdateCharacterVisual(EPartsType updateParts, FEquipItemInfo* equipItemInfo)
{
	...
	// 현재 장착된 파츠 중 updateParts 와 일치하는 파츠 정보를 얻음
	...
	{
		UE_LOG(LogTemp, Error, TEXT("PlayerInventory.cpp :: %d LINE :: Can not found matching updateParts"), __LINE__);
		return;
	}
...
void UPlayerInventory::MergeItem(UPlayerInventoryItemSlot* ori, UPlayerInventoryItemSlot* target)
{
	...
	// 둘 중 하나라도 최대 개수라면 swap이 일어나도록 함
	...
	else
	{
		...
		{
			// 옮길 개수를 슬롯 아이템 개수로 설정함
			addable = oriItemSlotInfo.ItemCount;
		}

		// 아이템을 옮김
		oriItemSlotInfo.ItemCount = oriItemSlotInfo.ItemCount - addable;
		targetItemSlotInfo.ItemCount = targetItemSlotInfo.ItemCount - addable;
