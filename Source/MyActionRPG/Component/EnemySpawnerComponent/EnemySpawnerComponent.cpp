...
UEnemySpawnerComponent::UEnemySpawnerComponent()
{
	...
	if (DT_ENEMY_SPAWN_REGION_INFO.Succeeded())
		DT_EnemySpawnRegionInfo = DT_ENEMY_SPAWN_REGION_INFO.Object;

	BoxExtent *= 10.0f;

	LineTraceStartHeight = BoxExtent.Z;
	// - BoxExtent : 육면체 크기 (X,Y,Z) / 2 의 값을 나타냄
...
void UEnemySpawnerComponent::InitializeSpawnPoints()
{
	...
	for (int32 y = 0; y < SpawnPointYCount; y++)
	{
		...
		{
			// 스폰 지점으로 사용될 작은 사각형들의 중심 위치를 얻음
...
void UEnemySpawnerComponent::RandomSpawn()
{
	...
	if (SpawnRegionInfo->EnemyCode.Num() == 0)
		return;

	if (SpawnRegionInfo->MaxEnemyCount == CurrentEnemyCount)
		return;
